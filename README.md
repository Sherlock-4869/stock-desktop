...
当前 MVP 包含：密码登录和安全会话恢复、服务端自选分组同步、定时行情刷新、可点击/拖动/缩放的置顶盯盘悬浮窗、窗口偏好恢复、`stockwatch://` 协议唤起，以及检查、下载、确认重启安装的自动更新流程。
...
首次打开客户端时填写部署好的 HTTPS 服务地址。开发时可将 `NODE_ENV=development` 并填写本地 HTTP 服务地址；发布版拒绝 HTTP。
