...
const { DESKTOP_SERVER_URL, PreferencesStore, validBounds } = require('../src/main/preferences-store');
...
test('the desktop service address is fixed even when an older preference file contains another URL', t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'stock-desktop-test-'));
  t.after(() => fs.rmSync(directory, { recursive:true, force:true }));
  fs.writeFileSync(path.join(directory, 'desktop-preferences.json'), JSON.stringify({
    serverUrl:'https://untrusted.example.test',
    marketColors:false,
  }));

  const settings = new PreferencesStore(directory).load();
  assert.equal(settings.serverUrl, DESKTOP_SERVER_URL);
  assert.equal(settings.marketColors, false);
});
